...
def split_subtitle(sub, wrapped_text):
    ...
    # Verifica se é uma única linha longa
    if len(lines) == 1 and len(lines[0]) > MAX_LINE_LENGTH:
        return [pysrt.SubRipItem(index=sub.index, start=sub.start, end=sub.end, text=wrapped_text)]

    if len(lines) <= 2:
        # Tenta manter a legenda inteira em duas linhas
        if len(' '.join(lines)) <= MAX_LINE_LENGTH * 2:
            new_text = '\n'.join(lines[:2])
            return [pysrt.SubRipItem(index=sub.index, start=sub.start, end=sub.end, text=new_text)]
